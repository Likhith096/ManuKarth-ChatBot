...
# Set the model path
MODEL_PATH = "Aditi25/InstructWithOurOwnData"
#Aditi25/results_HPE
#Aditi25/InstructWithOurOwnData
...
def generate_answer(question: str) -> str:
    ...
    response= response[response_start:].strip()
     ...
     # Remove "user" from the end of the response if it exists
...
#     with torch.inference_mode():
#         # Ensure the model is also moved to the GPU
#         model.to(device)
#         outputs = model.generate(
#             **encoding,
#             **config.__dict__  # Pass the generation config parameters to generate
